...
async function authSubmit(){
  ...
  try {
    ...
    if(window.authSignup){
      const { data: existingProfile, error: profileError } = await sb.from("profiles").select("id").eq("username", u).maybeSingle();
      ...
      result=await sb.auth.signUp({email,password:p,options:{data:{username:u}}});
    } else {
      result=await sb.auth.signInWithPassword({email,password:p});
    }
    ...
    // Do not perform database work from onAuthStateChange. Supabase can hold its
    // auth lock while firing that event, which can make the login button hang.
    state.session=result.data.session;
    if(!state.session) throw new Error("Login succeeded but no session was returned. Please refresh and try again.");
    ...
    if(state.session) message.textContent="";
  }
}
...
async function bootAuth(){
  try {
    ...
    // The callback only mirrors the session. It never calls Supabase database
    // methods, avoiding auth-lock races during login/logout.
...
async function createGame() {
  ...
  if (!state.session?.user?.id) { toast("Your login session is not ready. Please refresh and log in again."); return; }
  ...
  try {
    ...
    const { data: g, error } = await sb.from("games").insert({ name, sort_mode: "custom", owner_id: ownerId }).select().single();
    if (error) throw error;

    const rows = selected.map((pid, i) => ({ game_id: g.id, player_id: pid, score: 0, player_order: i, owner_id: ownerId }));
    const { error: e } = await sb.from("game_players").insert(rows);
    if (e) {
      // Roll back the empty game if its player links fail.
