...
async function authSubmit(){
  ...
  if(window.authSignup){
    ...
    if(profileError && !/row|permission|policy/i.test(profileError.message || "")){
      $("#authAction").disabled=false; $("#authMessage").textContent=profileError.message; return;
    }
    if(existingProfile){
      $("#authAction").disabled=false; $("#authMessage").textContent="That username is already in use."; return;
    }
...
async function bootAuth(){
  ...
  // Keep the auth callback synchronous. Supabase can deadlock its auth lock
  // when an async onAuthStateChange callback immediately performs Supabase calls.
  sb.auth.onAuthStateChange((_event,session)=>{
    state.session=session;
    setTimeout(async ()=>{
      if(session){
        await ensureProfile(session.user.user_metadata?.username);
        await loadAll();
      } else {
        state.profile=null;
        state.games=[];
