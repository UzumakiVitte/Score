(async function () {
  try {
    const response = await fetch("app-v7.js?v=10", { cache: "no-store" });
  ...
  try {
    ...
    await runWithTimeout(
      loadAll(), 15000,
      "Login succeeded, but loading your games timed out. Please try again."
    );
  } catch(err) {
  ...
  try {
    sb.auth.onAuthStateChange((_event,session)=>{
      ...
      if(!session){
        ...
        state.game=null; state.gamePlayers=[]; state.history=[];
        renderAuth();
      }
    });
    ...
    if(state.session){
      ...
      await runWithTimeout(
        loadAll(), 15000,
        "Your account loaded, but your games could not be loaded in time. Please try again."
      );
    } else {
      renderAuth();
    }
    ...
    if(msg) msg.textContent=err?.message || "Unable to connect to the account service.";
  }
}`;
  ...
  try {
    ...
    const recovery = `
const __scorekeeperShell = function(){
  const app=document.querySelector("#app");
  if(!app) return;
  if(!document.querySelector("#pageTitle") || !document.querySelector(".bottom-nav")){
    app.innerHTML =
      '<header class="topbar"><div><div class="eyebrow">CARD SCOREKEEPER</div><h1 id="pageTitle">Games</h1></div><button class="icon-btn" id="themeBtn" aria-label="Toggle theme">☀️</button></header>' +
      '<main id="content"></main>' +
      '<nav class="bottom-nav">' +
      '<button data-tab="games" class="nav-item active"><span>🎮</span><b>Games</b></button>' +
      '<button data-tab="players" class="nav-item"><span>👤</span><b>Players</b></button>' +
      '<button data-tab="history" class="nav-item"><span>🕘</span><b>History</b></button>' +
      '<button data-tab="settings" class="nav-item"><span>⚙️</span><b>Settings</b></button>' +
      '</nav>';
    document.querySelectorAll(".nav-item").forEach(function(b){
      b.onclick=function(){ closeModal(); state.tab=b.dataset.tab; render(); };
    });
    const theme=document.querySelector("#themeBtn");
    if(theme) theme.onclick=toggleTheme;
  }
};
setTitle=function(t){ const el=$("#pageTitle"); if(el) el.textContent=t; };
const __scorekeeperOriginalRender=render;
render=function(){
  __scorekeeperShell();
  __scorekeeperOriginalRender();
};
`;

    const script = document.createElement("script");
    script.textContent = code + recovery + "\nwindow.__scorekeeperVersion='v10';\nbootAuth();";
